...
const parseDocumentSize = (document2) => {
...
const U = 85;
const Z = 90;
const isDigit = (codePoint) => codePoint >= ZERO && codePoint <= 57;
...
const isLengthPercentage = (token) => token.type === 16 || isLength(token);
...
const deg = (deg2) => Math.PI * deg2 / 180;
const isTransparent = (color2) => (255 & color2) === 0;
const asString = (color2) => {
...
const pack = (r, g, b, a2) => (r << 24 | g << 16 | b << 8 | Math.round(a2 * 255) << 0) >>> 0;
...
const calculateGradientDirection = (angle2, width, height) => {
...
const distance = (a2, b) => Math.sqrt(a2 * a2 + b * b);
...
const parseDisplayValue$1 = (display2) => {
  switch (display2) {
    case "block":
    case "-webkit-box":
      return 2;
...
const computeLineHeight = (token, fontSize2) => {
...
const isVerticalWritingMode = (writingMode2) => writingMode2 !== 0;
const isSidewaysWritingMode = (writingMode2) => writingMode2 === 3 || writingMode2 === 4;
const writingMode = {
  name: "writing-mode",
...
const getQuote = (quotes2, depth, open) => {
  if (!quotes2) {
    return "";
  }
...
const parseDisplayValue = (display2) => {
  switch (display2) {
    case "contain":
      return 2;
    case "cover":
...
const testRangeBounds = (document2) => {
...
const testIOSLineBreak = (document2) => {
  const testElement = document2.createElement("boundtest");
...
const testForeignObject = (document2) => {
  const canvas = document2.createElement("canvas");
  const size = 100;
  canvas.width = size;
  canvas.height = size;
...
const wordSeparators = [32, 160, 4961, 65792, 65793, 4153, 4241];
...
const transform = (text, transform2) => {
  switch (transform2) {
    case 1:
...
const getCurvePoints = (x, y, r1, r2, position2) => {
...
const buildClipPathEffect = (clipPath2, bounds) => {
...
const calculateBackgroundPositioningArea = (backgroundOrigin2, element) => {
...
const calculateBackgroundPaintingArea = (backgroundClip2, element) => {
...
const canvasTextAlign = (textAlign2) => {
  switch (textAlign2) {
    case 1:
      return "center";
    case 2:
